...
export class AddNotificationComponent implements OnInit {
  ...
  toggleSelectAll() {
      // this.isSelect = !this.isAllSelected;
  ...
  insertNotificationDetails() {
    if (this.notificationForm.status == "INVALID") {
     ...
     for(let i=0;i<this.selectedUsers.length;i++){
        // console.log(i+"  "+this.selectedUsers.length)
        if(i == this.selectedUsers.length -1 ){
          this.userIds = this.userIds+this.selectedUsers[i].id;
        }else{
          this.userIds = this.userIds+this.selectedUsers[i].id+',';
        }
     }
      ...
      this.campusService.insertNotificationDetails(this.userIds, title, message,isAllSelected).subscribe((res) => {
        if (res.response_message == "Success") {
