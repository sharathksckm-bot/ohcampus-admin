...
export class ExamdocsComponent implements OnInit {
  ...
  getExamDetailsByIddd() {
      ...
      //  this.campusService.getExamDocById(this.page,this.pageSize,this.startNum,this.columnIndex,this.sortValue,this.docsForm.value.search).subscribe((res)=>{
  ...
  saveDoc() {
    ...
    this.campusService.SaveExamsDocs(examId,docType,docName,title,this.docs).subscribe((res) => {
      if (res.res_status == "Success") {
        ...
        this.addLoader = false
        Swal.fire('adf', res.response_message, 'error')
      }
    })
  }
  ...
  uploadMultipleDocuments(event, files: FileList) {
  ...
  // uploadMultipleDocuments(event,type){
  //   const formData = new FormData();
  //   let docName = this.docsForm.get('docType').value;

  //   if(docName == 'Question Paper'){
  //     this.showLoader = true;
  //     this.docs = [];
  //   }
